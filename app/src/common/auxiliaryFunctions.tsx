...
const showMessageWithHighlight = (
	message: string,
	...
	const messageHighlighted = words.map((word: string, index: number) => {
		...
		let symbolRemoved = ''
		if (
			commonSymbols.includes(word.substring(word.length - 1, word.length))
		) {
...
const filterLeavingOnlyNumbers = (dirtyText: string) => {
	...
	const cleanText = dirtyText.match(/[0-9]/gi)?.join('') || ''
	return cleanText
}
...
const formatRelativeDate = (daveValue: any) => {
	...
	const relative = formatRelative(date, currentDate, {
		locale: brasilLocale,
	})
...
const getNewDate = (date: any) => {
	if (
		Object.keys(date).includes('seconds')
		|| Object.keys(date).includes('_seconds')
	) {
...
const sortPostsByCreatedData = (
	a: PostCollectionRemote,
	b: PostCollectionRemote
) => {
