...
function SelectSaleCategory({
	...
	const renderSelectOptionsButtons = () => {
		const ordenedSaleCategories =
			Object.values(saleCategories).sort(sortPostCategories);
		...
		return ordenedSaleCategories.map((category, index) => {
			...
			return (
				<SelectButton
					...
					boldLabel
					onSelect={() =>
						onSelectCategory(category.value as SaleCategories)
					}
				/>
			);
		});
			...
			<ScrollView>
				<SelectButtonsContainer backgroundColor={theme.green2}>
					...
					<SelectButton
						...
						boldLabel
						onSelect={() =>
							onSelectCategory("others" as SaleCategories)
						}
					/>
