...
function InsertSocialImpactLocation({
	route,
	navigation,
}: InsertSocialImpactLocationScreenProps) {
	...
	const [mapContainerDimensions, setMapContainerDimensions] =
	...
	const [invalidAddressAfterSubmit, setInvalidAddressAfterSubmit] =
	...
	const getCurrentPositionCoordinated = async () => {
		...
		const currentPositionCoordinate =
	...
	const markerCoordinateIsAccuracy = () =>
		(markerCoordinate?.latitudeDelta as number) < 0.0065;
	...
	return (
		<Container behavior={Platform.OS === "ios" ? "padding" : "height"}>
			<StatusBar
				backgroundColor={
					someInvalidFieldSubimitted() ? theme.red2 : theme.pink2
				}
			...
			<LineInput
				...
				defaultBackgroundColor={
					validAddress ? theme.pink1 : theme.white3
				}
				defaultBorderBottomColor={
					validAddress ? theme.pink5 : theme.black4
				}
			...
			<MapContainer
				onLayout={(event: any) =>
					...
					setMapContainerDimensions(event.nativeEvent.layout)
				}
			>
				<View
					style={{
