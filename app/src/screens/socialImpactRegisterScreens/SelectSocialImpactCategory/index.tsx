...
function SelectSocialImpactCategory({
	route,
	navigation,
}: SelectSocialImpactCategoryScreenProps) {
	const { setSocialImpactDataOnContext } = useContext(SocialImpactContext);

	const renderSelectOptionsButtons = () => {
		const ordenedSocialImpactCategories = Object.values(
			socialImpactCategories
		).sort(sortPostCategories);
		...
		return ordenedSocialImpactCategories.map((category) => {
			...
			return (
				<SelectButton
					...
					onSelect={() =>
						onSelectCategory(
							category.value as SocialImpactCategories
						)
					}
				/>
			);
	...
	return (
		<Container>
			...
			>
				<SelectButtonsContainer backgroundColor={theme.pink2}>
					...
					<SelectButton
						...
						boldLabel
						onSelect={() =>
							onSelectCategory("others" as SocialImpactCategories)
						}
					/>
