...
function InsertEventEndHour({
	route,
	navigation,
}: InsertEventEndHourScreenProps) {
	...
	const closingTimeIsAfterOpening = () => {
		...
		const eventStartHour = new Date(
			cultureDataContext.eventStartDate as Date
		);
		const eventEndHour = new Date(cultureDataContext.eventEndDate as Date);
		eventStartHour.setHours(
			cultureDataContext.eventStartHour?.getHours() as number,
			cultureDataContext.eventStartHour?.getMinutes() as number
		);
	...
	const saveCulturePost = async () => {
		...
		try {
			...
			if (!culturePictures.length) {
				...
				await updateUserPost(localUser, postId, cultureData);
				return;
			}
								...
								getDownloadURL(uploadTask.snapshot.ref).then(
									async (downloadURL) => {
										...
										if (
											...
											culturePictures.length
										) {
											const cultureDataWithPicturesUrl = {
												...cultureData,
	...
	const extractCulturePictures = (cultureData: CultureData) =>
		(cultureData.picturesUrl as string[]) || [];
