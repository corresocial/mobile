...
function SearchResult({ route, navigation }: SearchResultScreenProps) {
	...
	const [profileResultsIsVisible, setProfileResultsIsVisible] =
		useState(false);
	...
	const searchPostByText = async () => {
		...
		await searchPostsCloud(
			...
			.then((posts) => {
				...
				if (!searchText)
					setSearchText(route.params.searchParams.searchText);
	...
	const getRelativePath = () => {
		...
		if (route.params.searchParams.category)
			return route.params.categoryLabel;
		...
		<Container>
			...
			<KeyboardAvoidingView style={{ flex: 1 }}>
				<Body
					style={{
						backgroundColor:
							locationDataContext.currentCategory.backgroundColor,
					}}
				>
					<FilterButtons>
						<SelectButton
					...
					{profileResultsIsVisible &&
						(!resultProfiles.length ? (
							<WithoutPostsMessage
								title={"poxa!"}
								message={
									"parece que não temos nenhum usuário com esse nome no corre."
								}
							/>
						) : (
							<WithoutPostsMessage
