...
function InsertWorkEndHour({
	route,
	navigation,
}: InsertWorkEndHourScreenProps) {
	...
	const closingTimeIsAfterOpening = () => {
		...
		const startWorkHour = new Date(
			(vacancyDataContext.startWorkDate as Date) || new Date()
		)
		const endWorkHour = new Date(
			(vacancyDataContext.endWorkDate as Date) || new Date()
		)
		startWorkHour.setHours(
			vacancyDataContext.startWorkHour?.getHours() as number,
			vacancyDataContext.startWorkHour?.getMinutes() as number
		)
	...
	const saveVacancyPost = async () => {
		...
		try {
			...
			await updateUserPost(localUser, postId, vacancyData)
			return
		} catch (err) {
			console.log(err)
			setIsLoading(false)
	...
	return (
		<Container behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
			<StatusBar
				backgroundColor={
					invalidTimeAfterSubmit ? theme.red2 : theme.yellow2
				}
