...
function SelectWorkWeekdays({
	route,
	navigation,
}: SelectWorkWeekdaysScreenProps) {
	...
	const onSelectDay = (dayOfWeek: DaysOfWeek) => {
		...
		if (selectedDays.includes(dayOfWeek)) {
			const selectedDaysFiltred = selectedDaysOfWeek.filter(
				(day) => day !== dayOfWeek
			);
	...
	const saveWorkWeekdays = () => {
		if (editModeIsTrue()) {
			addNewUnsavedFieldToEditContext({ workWeekdays: selectedDays });
			navigation.goBack();
			return;
		}

		setVacancyDataOnContext({ workWeekdays: selectedDays });
