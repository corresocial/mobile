...
function SelectServiceCategory({
	route,
	navigation,
}: SelectServiceCategoryScreenProps) {
	const { setServiceDataOnContext } = useContext(ServiceContext);

	const renderSelectOptionsButtons = () => {
		const ordenedServicesCategories =
			Object.values(serviceCategories).sort(sortPostCategories);
		...
		return ordenedServicesCategories.map((category) => {
			...
			return (
				<SelectButton
					...
					boldLabel
					onSelect={() =>
						onSelectCategory(category.value as ServiceCategories)
					}
				/>
			);
		});
	...
	return (
		<Container>
			...
			<ScrollView>
				<SelectButtonsContainer backgroundColor={theme.purple2}>
					...
					<SelectButton
						...
						boldLabel
						onSelect={() =>
							onSelectCategory("others" as ServiceCategories)
						}
					/>
