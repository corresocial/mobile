...
function filterValidEvents(events: EventPost[], registeredExternalEvents: EventPost[]) {
    const validEvents = events.filter((event) => {
        ...
        if (!event.links || (event.links && !event.links[0])) return false
        if (!event.location || (
            event.location
            && (
...
function removeEventsAlreadyPosted(newExternalEvents: EventPost[], registeredExternalEvents: EventPost[]) {
...
function filterInvalidEvents(events: EventPost[], registeredEvents: EventPost[]) {
    ...
    const registeredEventsToRemove = registeredEvents.filter((registeredEvent) => {
        if (!events.some((event) => registeredEvent.externalPostId === event.externalPostId)) return true
        return false
    })
...
export { filterValidEvents, filterInvalidEvents }